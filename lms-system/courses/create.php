...
// Hämta alla användare för sökfunktion (till JS)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $pdo->prepare("INSERT INTO lms_courses 
        (title, short_description, image, info, show_in_catalog, date_from, date_to, keywords, admin_id, responsible_id, certificate_text, certificate_responsible, created_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())");
    $stmt->execute([
        $title, $short_description, $image, $info, $show_in_catalog, $date_from, $date_to, $keywords,
        $admin_id, $responsible_id, $certificate_text, $certificate_responsible
    ]);
    echo "<div class='alert alert-success'>" . t('course_created') . "</div>";
    // exit; // Ta bort exit om du vill visa formuläret igen efter skapande
}
?>
<!DOCTYPE html>
...
<head>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function setupUserSearch(inputId, hiddenId, userList) {
        ...
        setupUserSearch('admin_search', 'admin_id', userList);
        setupUserSearch('responsible_search', 'responsible_id', userList);
...
<div class="profile-container" style="max-width:600px;margin:40px auto;">
    ...
    <form method="POST" enctype="multipart/form-data" class="login-form" autocomplete="off">
        ...
        <label>
            <input type="checkbox" name="show_in_catalog" value="1"> <?= t('show_in_catalog') ?>
        </label>
        <label><?= t('date_from') ?>
        ...
        <textarea id="certificate_text" name="certificate_text" maxlength="255" rows="3" style="resize:vertical; width:100%;"></textarea>
        ...
        <label><?= t('certificate_responsible') ?>
            <input type="text" name="certificate_responsible" maxlength="255">
        </label>
